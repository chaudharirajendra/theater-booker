...
func InitializeTheater() *theater.Theater {
	// Initialize theater with shows and available seats
	return theater.NewTheater([]theater.Show{
		{
...
func ScanShowNo(prompt string) (int, error) {
	var n int
	fmt.Print(prompt)
	_, err := fmt.Scanln(&n)
	if err != nil {
...
func ParseSeatInputs(seatInput string) ([]theater.Seat, error) {
	...
	for _, seatStr := range seatStrings {
		...
		if len(seatStr) < 2 {
			...
			return seats, fmt.Errorf("invalid seat format: %s", seatStr)
		}
